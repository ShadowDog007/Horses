...
public class HorseSpeedUtil
{
	...
	private static void setup(Horse horse)
	{
		...
		try
		{
			...
			try
			{
				genAttributes = Class.forName("net.minecraft.server.v1_6_R" + version + ".GenericAttributes");
			}
			catch (Throwable e)
			{
				if (++version > 5)
			...
			Class<?> iattribute = Class.forName("net.minecraft.server.v1_6_R" + version + ".IAttribute");
